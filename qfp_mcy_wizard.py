...
# copy from qfp_wizard.py
class QFPWizard(FootprintWizardBase.FootprintWizard):

    def GetName(self):
        return "QFP(Rectangle)"

    def GetDescription(self):
        return "Quad Flat Package (QFP) footprint wizard, for Rectangle"

    def GenerateParameterList(self):
        self.AddParam("Pads", "n", self.uInteger, 100, multiple=4, min_value=4)
        self.AddParam("Pads", "pitch", self.uMM, 0.65, designator='e')
        self.AddParam("Pads", "width", self.uMM, 0.3, designator='X1')
        self.AddParam("Pads", "length", self.uMM, 2.1, designator='Y1')
        self.AddParam("Pads", "horizontal spacing", self.uMM, 16.2, designator='C1')
        self.AddParam("Pads", "vertical spacing", self.uMM, 22.2, designator='C2')
        self.AddParam("Pads", "oval", self.uBool, True)
        self.AddParam("Pads", "horizontal pins", self.uInteger, 20)
        self.AddParam("Pads", "vertical pins", self.uInteger, 30)

        self.AddParam("Package", "width", self.uMM, 14.1, designator='D1')
        self.AddParam("Package", "height", self.uMM, 20.1, designator='E1')
    ...
    def BuildThisFootprint(self):
        ...
        pads_horizontal_row = int(self.pads["horizontal pins"])     # 水平腳數
        pads_vertical_row = int(self.pads["vertical pins"])         # 垂直腳數

        row_len = (pads_per_row - 1) * pad_pitch
        row_horizontal_len = (pads_horizontal_row - 1) * pad_pitch
        row_vertical_len = (pads_vertical_row - 1) * pad_pitch

        pad_shape = pcbnew.PAD_SHAPE_OVAL if self.pads["oval"] else pcbnew.PAD_SHAPE_RECT

        h_pad = PA.PadMaker(self.module).SMDPad( pad_length, pad_width, shape=pad_shape, rot_degree=90.0)
        ...
        array = PA.PadLineArray(h_pad, pads_vertical_row, pad_pitch, True, pin1Pos)
        ...
        array = PA.PadLineArray(v_pad, pads_horizontal_row, pad_pitch, False, pin1Pos)
        array.SetFirstPadInArray(pads_horizontal_row + 1)
        array.AddPadsToModule(self.draw)
        ...
        array = PA.PadLineArray(h_pad, pads_vertical_row, -pad_pitch, True, pin1Pos)
        array.SetFirstPadInArray(2*pads_vertical_row + 1)
        array.AddPadsToModule(self.draw)
        ...
        array = PA.PadLineArray(v_pad, pads_horizontal_row, -pad_pitch, False, pin1Pos)
        array.SetFirstPadInArray(3*pads_horizontal_row+ 1)
        array.AddPadsToModule(self.draw)
        ...
        inner_horizontal = (row_horizontal_len / 2) + pad_pitch
        inner_vertical = (row_vertical_len / 2) + pad_pitch
        ...
        #self.draw.Polyline([(-inner_horizontal, -y), (-x, -y), (-x, -inner_vertical), (left_edge, -inner_vertical)])
        # top right
        #self.draw.Polyline([(inner, -y), (x, -y), (x, -inner)])
        self.draw.Polyline([(inner_horizontal, -y), (x, -y), (x, -inner_vertical)])
        # bottom left
        #self.draw.Polyline([(-inner, y), (-x, y), (-x, inner)])
        self.draw.Polyline([(-inner_horizontal, y), (-x, y), (-x, inner_horizontal)])
        # bottom right
        #self.draw.Polyline([(inner, y), (x, y), (x, inner)])
        self.draw.Polyline([(inner_horizontal, y), (x, y), (x, inner_horizontal)])
